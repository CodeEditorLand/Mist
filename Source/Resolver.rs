...
//! Routes `*.editor.land` queries to loopback; other domains fall back to system DNS.
...
pub fn LandResolver(_DNSPort: u16) -> TokioResolver {
	TokioResolver
}
...
impl reqwest::dns::Resolve for LandDnsResolver {
	fn resolve(&self, Name: reqwest::dns::Name) -> reqwest::dns::Resolving {
		...
		Box::pin(async move {
			let IsEditorLand =
				NameString.ends_with(".editor.land") || NameString == "editor.land";
			if IsEditorLand {
				let Addresses =
...
mod tests {
	...
	#[test]
	fn TestResolverCreation() {
		let _Resolver = LandResolver(15353);
	}

	#[test]
	fn TestLandDnsResolverCreation() {
		let _Resolver = LandDnsResolver::New(15354);
	}
